...
def create_base_url():
	...
	base_url_pool = [url]
	for i in range(2,31):
		creat_url = 'http://news.yule.com.cn/yingshi/index%d.html'%(i)
...
async def get_url(base_url,centure_url_lilst):
	...
	page_url = two_html.xpath('//div[@class="titname"]/a/@href')
	...
	try:
		for i in get_centure_url():
			if 'http' not in str(i):
				lastUrl = 'http://news.yule.com.cn/yingshi' + str(i)
			else:
				lastUrl = str(i)
...
def main():
	...
	for open_url,article_id in zip(create_last_url([]),range(last_id+1,99999)):
		try:
			...
			appid = 15
			article_name = thrid_html.xpath('//div[@class="CBArticle"]/h1/text()')[0]
			create_date = datetime.now()
			create_idate = int(str(create_date.date()).replace('-',''))
			# 内容标题
			article = thrid_html.xpath('//div[@id="NewsContentLabel"]/p//text() | //div[@id="NewsContentLabel"]//img')
			...
			# 图片处理
			images_url = thrid_html.xpath('//div[@id="NewsContentLabel"]//img/@src')
			images_url = list(set(images_url))

			image_title = thrid_html.xpath('//div[@id="NewsContentLabel"]//img/@title')
