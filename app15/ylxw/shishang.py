# 抓取环球时尚　http://fashion.huanqiu.com/news/
import requests
...
def create_base_url():
	base_url_pool = [url]
	for i in range(2,11):
		creat_url = 'http://fashion.huanqiu.com/news/%d.html'%(i)
...
async def get_url(base_url,centure_url_lilst):
	...
	page_url = two_html.xpath('//div[@class="fallsFlow"]//li/a/@href')
	...
	try:
		for i in get_centure_url():
			if 'http' not in str(i):
				lastUrl = 'http://fashion.huanqiu.com/news' + str(i)
			else:
				lastUrl = str(i)
...
def main():
	...
	for open_url,article_id in zip(create_last_url([]),range(last_id+1,99999)):
		try:
			...
			appid = 15
			article_name = thrid_html.xpath('//div[@class="l_a"]/h1/text()')[0]
			create_date = datetime.now()
			create_idate = int(str(create_date.date()).replace('-',''))
			# 内容标题
			article = thrid_html.xpath('//div[@class="la_con"]//p/text() | //div[@class="la_con"]//p//span/text() | //div[@class="la_con"]//p//img | //div[@class="la_con"]//p/strong/text()')
			contents,imageNum = to_img(article)

			del_head = ''.join(contents).replace('【环球网综合报道】','')
			re_end = re.compile('\(实习编译.*')
			content = re_end.sub('',del_head)
			print(content)
			# class_id = 112233
			...
			# 图片处理
			images_url = thrid_html.xpath('//div[@class="la_con"]//p/img/@src')
			images_url = list(set(images_url))

			image_title = thrid_html.xpath('//div[@class="la_con"]//p/img/@title')
			...
			if images_url:
				...
				for image_url in images_url:
					...
					if 'http' in image_url:
						...
						image_url = 'http://fashion.huanqiu.com/news' + image_url
...
if __name__ == '__main__':
	...
	url = 'http://fashion.huanqiu.com/news/'
