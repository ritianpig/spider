# 影视爬取网址：http://www.xinwentoutiao.net/yule/ 北青网娱乐
import requests
...
def create_base_url():
	base_url_pool = [url]
	for i in range(2,61):
		creat_url = url + 'list_5_{}.html'.format(str(i))
...
async def get_url(base_url,centure_url_lilst):
	...
	page_url = two_html.xpath('//a[@class="J-share-a"]/@href')
	...
	try:
		for i in get_centure_url():
			if 'http' not in str(i):
				lastUrl = 'http://www.xinwentoutiao.net/yule' + str(i)
			else:
				lastUrl = str(i)
...
def main():
	...
	if last_id is None:
		...
		last_id = last_id[0]
	print(last_id)
	print(create_last_url([]))
	print(get_centure_url())
	for open_url,article_id in zip(create_last_url([]),range(last_id+1,99999)):
		try:
			...
			appid = 15
			article_name = thrid_html.xpath('//div[@class="J-title_detail title_detail"]//h1//text()')[0]
			create_date = datetime.now()
			create_idate = int(str(create_date.date()).replace('-',''))
			# 内容标题
			article = thrid_html.xpath('////div[@class="J-contain_detail_cnt contain_detail_cnt"]//text() | //div[@class="J-contain_detail_cnt contain_detail_cnt"]//img')
			...
			# 图片处理
			images_url = thrid_html.xpath('//div[@class="J-contain_detail_cnt contain_detail_cnt"]//img/@src')
			images_url = list(set(images_url))

			image_title = thrid_html.xpath('//div[@class="J-contain_detail_cnt contain_detail_cnt"]//img/@title')
			...
			if images_url:
				...
				for image_url in images_url:
					...
					if 'http' in image_url:
						...
						image_url = 'http://www.xinwentoutiao.net/yule' + image_url
...
if __name__ == '__main__':
	...
	url = 'http://www.xinwentoutiao.net/yule/'
