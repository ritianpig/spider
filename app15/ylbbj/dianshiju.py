# 采集　　http://www.hxnews.com/news/yl/kdsj/ 海峡网，看电视剧频道
import requests
...
db= pymysql.connect(host="localhost",user="root",
 	password="129426",db="yuleBBji",port=3306)

cur = db.cursor()

url = 'http://www.hxnews.com/news/yl/kdsj/'
...
# 根据网站特性，构造url池,抓取前60页内容
url_pool = [url]
for i in range(2,51):
	creat_url = creat_url = 'http://www.hxnews.com/news/yl/kdsj/%d.shtml'%(i)
...
for i in url_pool:
	...
	page_url = two_html.xpath('//div[@class="photobox"]//a/@href')
	Paging += page_url
	time.sleep(2)
...
for open_url,article_id in zip(Paging,range(1,100000)):
	try:
		...
		appid = 15
		article_name = thrid_html.xpath('//div[@class="page-header"]//h1/text()')[0]

		create_date = datetime.now()
		create_idate = int(str(create_date.date()).replace('-',''))
		# 内容标题
		article = thrid_html.xpath('//div[@class="article article_16 article-content fontSizeSmall"]//p//text() | //div[@class="article article_16 article-content fontSizeSmall"]//p//img')
		...
		# 图片处理
		images_url = thrid_html.xpath('//div[@class="article article_16 article-content fontSizeSmall"]//p//img/@src')
		images_url = list(set(images_url))

		image_title = thrid_html.xpath('//div[@class="article article_16 article-content fontSizeSmall"]//p//img/@title')
		if not image_title:
